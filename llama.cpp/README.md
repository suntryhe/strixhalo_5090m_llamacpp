...
## This fork: Strix Halo + RTX 5090 dual-backend inference / 双后端分支说明

**EN** — llama.cpp for split inference on one machine: AMD Strix Halo (Ryzen AI Max 395, gfx1151) + NVIDIA RTX 5090, one `llama-server`, two backends (CUDA attention + ROCm experts). Focused on `qwen4exp` MoE-hybrid models (Qwen3.8-Flash-Next) that use lightning-indexer QSA sparse attention. Based on upstream `main` (~Aug 2026, post #27886); all fork changes are additive or env-gated.

**中文** — 单机双卡推理分支：AMD Strix Halo（Ryzen AI Max 395，gfx1151）+ RTX 5090，单 `llama-server` 进程双后端（CUDA 跑 attention，ROCm 跑 MoE 专家）。面向 `qwen4exp` 混合架构（Qwen3.8-Flash-Next）与 lightning-indexer QSA 稀疏注意力。基于上游主线（2026-08 末，#27886 之后），全部改动为增量式或带环境变量开关。

### Changes vs upstream / 与主线差异（22 commits）

**Dual-backend execution / 双后端执行**
...
- **hot-block gather prefill** (this fork's headline): prefill attention over one shared hot window per layer - block-level top-k over indexer scores + the chunk's own cells; replaces the `[n_tps, n_kv]` mask-expansion chain. `LLAMA_QSA_HOTBLOCK` (default on, `0` disables), `LLAMA_QSA_HOTBLOCK_CELLS` (history budget, default 2048), `LLAMA_QSA_DISABLE=1` = plain full attention ground truth
- NextN/MTP draft head (`--spec-type draft-mtp`)

**Env switches / 环境开关一览**: `LLAMA_QSA_DISABLE`, `LLAMA_QSA_HOTBLOCK`, `LLAMA_QSA_HOTBLOCK_CELLS`, `LLAMA_QSA_GATHER`, `LLAMA_QSA_NO_POOLED_CACHE`, `LLAMA_QSA_HOTBLOCK_DEBUG`, `GGML_HIP_DISABLE_GRAPHS`, `GGML_HIP_MEM_MODE`, `LLAMA_EXPERT_LRU_HARVEST_EVERY`

### Measured gains (Qwen3.8-Flash-Next-UD-IQ4_XS, APU 8060S / 实测)
...
### Configs & launch commands / 配置与启动命令
...
**Qwen3.8-Flash-Next (qwen4exp), 5090 + Strix Halo dual-card / 双卡:**
...
**DeepSeek-V4-Flash (MoE), 5090 + Strix Halo dual-card / 双卡:**
